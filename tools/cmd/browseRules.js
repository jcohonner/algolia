...
module.exports = class browseRules extends AlgocliScript {
  ...
  async run() {
          ...
          ...hits.filter(
            (rule) =>
              rule.conditions?.some(
                (condition) =>
                  condition.context && condition.context.startsWith("rayon")
              ) &&
    ...
    rules.forEach((rule) => {
      logOutput.push(
        `${rule.objectID},${rule.conditions
          ?.filter((condition) => condition.context?.startsWith("rayon"))
          .map((condition) => condition.context)
          .join(" ")},${
          rule.description
        },"${rule.consequence.params.filters.replace(/"/g, "")}"`
      );
    });
    ...
    const fs = require("fs");
    fs.writeFileSync(
      `output/${this.indexName}-rules-products.csv`,
      logOutput.join("\n")
    );
  }
};
